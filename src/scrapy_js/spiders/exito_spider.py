...
class SpiderExito(scrapy.Spider):
    name = "exito"
    custom_settings = {"FEED_URI":os.path.join(os.getcwd(),"src/assets/results.json")}
            ...
            for sub, links in sub_dict.items():
                for link in links[0:2]:
                    yield response.follow(
                        url=link+"?page=1",
                        callback = self.parse, 
                        cb_kwargs = {"sub_name" :sub_name,"cat_name":cat, "products":[]}
                        )
    ...
    def parse(self,response:Response, **kwargs):
        ...
        try: 
            ...
            for val in values:
                kwargs.update({
                    "name":val.xpath(XPATH_NAME_ELEMENT).get(),
                    "price":val.xpath(XPATH_PRICE_ELEMENT).get()})
            print(kwargs)
            comp = re.compile("(?<=page=)\d+(?=.)*")
            page_num = int(comp.search(driver.current_url).group(0)) + 1
            new_url = re.sub("(?<=page=)\d+(?=.)*",str(page_num), driver.current_url)
