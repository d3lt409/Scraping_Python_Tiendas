...
# para definir la categoría o subcategoría o subsub...
from selenium.webdriver.common.by import By
from selenium.webdriver.support.ui import WebDriverWait
from src.scraper.engine.engine import CLICK  # para hacer click
# scrapping creada por nosotros (Manuel)
...
import sys
sys.path.append(".")  # para enteder q las carpetas son modulos
...
def categoires():
    """
    Esto me trae las categorias de los productos
    """
    global lenth
    # lenth = {"mi_vector" : [1,2,3,4,5], "papas":{"fritas":"sdlkfjsd"}}
    # lenth["mi_vector"] = lenth.get("mi_vector")*4
    ...
    cat_list_object = WebDriverWait(driver, TIME_OUT).until(EC.presence_of_all_elements_located(
        (By.XPATH, "//li[@class='ant-menu-submenu ant-menu-submenu-vertical category-level-1']")))  # categories list
    ...
    driver.execute_script(CLICK, WebDriverWait(driver, TIME_OUT).until(EC.presence_of_element_located(
        (By.XPATH, f"//li[@class='ant-menu-submenu ant-menu-submenu-vertical category-level-1'][contains(text(),'{category}')]"))))  # Click on the first category
    driver.execute_script(CLICK, WebDriverWait(driver, TIME_OUT).until(EC.presence_of_element_located(
...
def get_elements(cat, list_elements: list):
    ...
    elements = driver.find_elements(
        By.XPATH, "//div[@class= 'card-product-vertical ']")
    ...
    for el in elements:  # Definir un loop sobre los elementos descargados para un producto
        ...
        list_elements.append((nombre, cat, float(precio), int(
            cant_uni_prec[0]), cant_uni_prec[1], float(cant_uni_prec[2]), datetime.now().date()))
    try:
        driver.execute_script(CLICK, WebDriverWait(driver, 3).until(EC.presence_of_element_located(
            (By.XPATH, "//li[@title='Next Page'][@aria-disabled='false']"))))
        return get_elements(cat, list_elements)
    except TimeoutException as _:
        df = pd.DataFrame(list_elements, columns=[
                          "Nombre_producto", "Categoria", "Precio", "Cantidad", "Unidad", "Precio_unidad", "Fecha_resultados"])
        db.to_data_base(df)
        lenth["Cantidad elementos"] = lenth.get(
            "Cantidad elementos", 0) + len(list_elements)
        print("se guardan los archivos de la categoría",
              cat, "la cantidad de ", len(df))
