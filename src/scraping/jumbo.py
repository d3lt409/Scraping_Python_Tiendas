...
def each_departments_categories():
    ...
    while True:
        try:
            dep_button: WebElement = engine.element_wait_searh(
                20, By.XPATH, "//*[@id='menu-item-music-store']")
    ...
    for el in dep_element_object:
        el = engine.element_wait_searh(
            10, By.XPATH, f"//ul[@class='tiendasjumboqaio-jumbo-main-menu-2-x-nav_menu tiendasjumboqaio-jumbo-main-menu-2-x-nav_menu--header-submenu-item']/div/div/li//a[contains(text(),'{el}')]")
        text = el.text
        ActionChains(engine.driver).move_to_element(el).perform()
        time.sleep(1)
        try:
            elementos = engine.elements_wait_searh(
                5, By.XPATH,  "//div[@class='tiendasjumboqaio-jumbo-main-menu-2-x-submenus_wrapper tiendasjumboqaio-jumbo-main-menu-2-x-submenus_wrapper--header-submenu-item']/div/div/ul/li/div")
...
def get_subcategories(dep, cat, sub):
    ...
    while True:
        if (url.__contains__("page=")):
            engine.current_url = re.sub("\d+$", "", url)+str(count)
        elif (url.__contains__("?")):
            engine.current_url = f"{url}&page={count}"
        else:
            engine.current_url = f"{url}?page={count}"
        try:
            ...
            try:
                engine.element_wait_searh(
                    2, By.XPATH, "//div[@class='vtex-search-result-3-x-searchNotFoundInfo flex flex-column ph9']")
                return
            except TimeoutException:
                pass
            try:
                engine.element_wait_searh(
                    2, By.XPATH, "//p[@class='lh-copy vtex-rich-text-0-x-paragraph vtex-rich-text-0-x-paragraph--not-found-opps']")
                return
            except TimeoutException as e:
                count += 1
                continue
...
def get_elements(dep, cat, subcat):
    ...
    print(
        f"Productos guardados, Departamento {dep} en la categoría: {cat}, subcategoría: {subcat} a las {datetime.now()}, la cantidad de {len(df)} productos")
    ...
    try:
        ...
        if len(cant) == 0:
            ...
            return re.search("\d+[(,|\.)\d+]*", cant[0]).group(0).replace(",", ".").strip(), \
                re.search("(?<=[xX])?(?<=[0-9] )?[a-zA-Z]+$",
                          cant[0]).group(0).strip()
        elif len(cant) > 1:
            return re.search("\d+[(,|\.)\d+]*", cant[-1]).group(0).replace(",", ".").strip(), \
                re.search("(?<=[xX])?(?<=[0-9] )?[a-zA-Z]+$",
                          cant[-1]).group(0).strip()
...
def main():
    ...
    time.sleep(3)
    lenth["Cantidad"] = engine.db.consulta_sql_unica(
        "select count(*) from D1;")[0]
    heigth = engine.element_wait_searh(
        15, By.XPATH, "//div[@class='vtex-store-footer-2-x-footerLayout']").size["height"]
