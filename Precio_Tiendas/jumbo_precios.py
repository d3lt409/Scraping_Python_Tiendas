...
from Util import consulta_sql, consulta_sql_unica, crash_refresh_page, init_scraping,ready_document, to_data_base
...
def consulta_ultima_peticion():
    fecha = consulta_sql_unica("select max(Fecha_resultados) FROM Jumbo;")
    hora = consulta_sql_unica(f"select max(Hora_resultados) FROM Jumbo where Fecha_resultados = {fecha!r};")
    ult_dep = consulta_sql_unica(f"SELECT Departamento from Jumbo where Hora_resultados = {hora!r} and Fecha_resultados = {fecha!r};")
    ult_cat = consulta_sql_unica(f"SELECT Categoria from Jumbo where Hora_resultados = {hora!r} and Fecha_resultados = {fecha!r} and Departamento= {ult_dep!r};")
    ult_sub = consulta_sql_unica(f"""SELECT Sub_categoria from Jumbo where Hora_resultados = {hora!r} and Fecha_resultados = {fecha!r} 
            and Departamento= {ult_dep!r} and Categoria = {ult_cat!r};""")
    
    deps = [val[0] for val in consulta_sql(f"select DISTINCT Departamento FROM Jumbo where Departamento <> {ult_dep!r} and Fecha_resultados = {fecha!r};")
                if val[0]] 
    cats = [val[0] for val in consulta_sql(f"select DISTINCT Categoria FROM Jumbo where Departamento = {ult_dep!r} and Categoria <> {ult_cat!r} and Fecha_resultados = {fecha!r};")
                if val[0]]
    subs = [val[0] for val in consulta_sql(f"""select DISTINCT Sub_categoria FROM Jumbo where Departamento = {ult_dep!r} and Categoria = {ult_cat!r}
...
driver = init_scraping(current_url_jumbo,"Jumbo")
